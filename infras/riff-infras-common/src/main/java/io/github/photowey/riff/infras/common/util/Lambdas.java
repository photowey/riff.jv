...
public final class Lambdas {
    ...
    private Lambdas() {
        AssertionErrors.throwz(Lambdas.class);
    }
    ...
    public static <T, M, D> List<D> toFlapList(
        Collection<T> from,
        Function<T, Stream<M>> mapper,
        Function<M, D> fx) {
        return from.stream()
            .flatMap(mapper)
            .map(fx)
    ...
    public static <T, D> Set<D> toSet(
        Collection<T> from, Function<T, D> mapper) {
        return from.stream()
            .map(mapper)
            .filter(Objects::nonNull)
    ...
    public static <T, D> Set<D> toSet(
        Collection<T> from, Predicate<T> predicate,
        Function<T, D> mapper) {
        return from.stream()
            .filter(predicate)
            .map(mapper)
    ...
    public static <T, M, D> Set<D> toFlapSet(
        Collection<T> from,
        Function<T, Stream<M>> mapper,
        Function<M, D> fx) {
        return from.stream()
    ...
    public static <K, T extends Collection<V>, V> List<V> flapMapList(
        Collection<K> from,
        Function<K, T> tMapper) {
        return flapMapList(from, tMapper, Collection::stream);
    }

    public static <K, T, V> List<V> flapMapList(
        Collection<K> from,
        Function<K, T> tMapper,
        Function<T, Stream<V>> vMapper) {
        return from.stream()
            .map(tMapper)
            .flatMap(vMapper)
            .collect(Collectors.toList());
    ...
    public static <K, T extends Collection<V>, V> Set<V> flapMapSet(
        Collection<K> from,
        Function<K, T> tMapper) {
        return flapMapSet(from, tMapper, Collection::stream);
    }

    public static <K, T, V> Set<V> flapMapSet(
        Collection<K> from,
        Function<K, T> tMapper,
        Function<T, Stream<V>> vMapper) {
        return from.stream()
    ...
    public static <T> void noOps(T t) { }

    public static <T, U> void noOps(T t, U u) { }

    public static <T, U, V> void noOps(T t, U u, V v) { }

    public static <T, U, V, R> void noOps(T t, U u, V v, R r) { }

    public static <T, U, V, R, W> void noOps(T t, U u, V v, R r, W w) { }
