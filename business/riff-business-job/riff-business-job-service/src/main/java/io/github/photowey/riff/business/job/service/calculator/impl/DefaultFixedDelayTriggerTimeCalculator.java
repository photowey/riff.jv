...
import io.github.photowey.riff.business.job.service.calculator.FixedDelayTriggerTimeCalculator;
...
/**
 * {@code DefaultFixedDelayTriggerTimeCalculator}.
 *
 * @author photowey
 * @version 1.0.0
 * @since 2025/08/01
...
@Slf4j
@Component
public class DefaultFixedDelayTriggerTimeCalculator
    extends AbstractBeanFactoryHolder implements FixedDelayTriggerTimeCalculator {
    ...
    private void report(ScheduleContext ctx, ScheduleJob job, LocalDateTime nextTime) {
        if (log.isInfoEnabled()) {
            log.info("riff: Attempting to calculate next trigger time for fixed-delay job [{}]. Now: [{}], Next: [{}]",
                ctx.query(),
